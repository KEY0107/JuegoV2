...
class InteriorEdificioH2Scene(Scene):
    def __init__(self, screen, player):
        ...
        self.obstacles = get_collisions("hallway_h2")
        self.player = player
        ...
        self.DEFAULT_ZOOM = 2
        self.current_map = "interior_edificio_h2"
    ...
    def update(self, dt):
        ...
        # transicion para regresar al piso 1
        door_interior_to_edificio_h = pygame.Rect(344, 490, 93, 10)
        if self.player.rect.colliderect(door_interior_to_edificio_h):
            self.player.rect.center = (280, 411)
            return "interior_edificio_h"
        # transicion para entrar al salon
        door_interior_to_salon = pygame.Rect(705, 230, 40, 10)
        if self.player.rect.colliderect(door_interior_to_salon):
            self.player.rect.center = (557, 401)
            return "salon_h1"

        # transicion para entrar al salon 2
        door_interior_to_salon2 = pygame.Rect(701, 352, 50, 5)
        if self.player.rect.colliderect(door_interior_to_salon2):
            self.player.rect.center = (165, 94)
            return "salon_h2"
