...
from settings import SOUND_DIR, ASSETS_DIR
...
class SalonH1Scene(Scene):
    def __init__(self, screen, player):
        ...
        self.class_event_active = False  # Evento de fin de clase (pantalla negra con "Fin de clase")
        ...
        # Cargar el sonido del aula usando la ruta centralizada
        sound_file = os.path.join(SOUND_DIR, "classroom.mp3")
        self.classroom_sound = pygame.mixer.Sound(sound_file)
    ...
    def update(self, dt):
        ...
        if self.class_event_active:
            ...
            return None  # Congela actualización normal durante el evento
        ...
        # Rectángulo que activa el evento de fin de clase (se activa solo la primera vez)
        event_rect = pygame.Rect(470, 391, 162, 6)
        if (not globales_chapter_1.FIN_CLASE_EVENTO_ACTIVADO and
                self.player.rect.colliderect(event_rect)):
            self.class_event_active = True
            globales_chapter_1.FIN_CLASE_EVENTO_ACTIVADO = True
            self.class_event_timer = 4000  # 4 segundos
    ...
    def get_hud_visibility(self):
        # Ocultar el HUD durante eventos especiales
        if self.class_event_active or self.cellphone_event_active:
            return False
        return True

    def render(self):
        # Evento de fin de clase: pantalla negra con "Fin de clase" y "9:00pm"
        if self.class_event_active:
            self.screen.fill((0, 0, 0))
            font = pygame.font.SysFont("arial", 48, bold=True)
            text1 = font.render("Fin de clase", True, (255, 255, 255))
            text2 = font.render("9:00pm", True, (255, 255, 255))
            self.screen.blit(text1, ((self.screen.get_width() - text1.get_width()) // 2,
                                      self.screen.get_height() // 2 - 30))
            self.screen.blit(text2, ((self.screen.get_width() - text2.get_width()) // 2,
                                      self.screen.get_height() // 2 + 30))
            pygame.display.flip()
            return

        # Evento celular: mostrar imagen del celular y diálogo de Alex
        if self.cellphone_event_active:
            self.screen.fill((0, 0, 0))
            try:
                # Construir la ruta del asset usando ASSETS_DIR
                cellphone_path = os.path.join(ASSETS_DIR, "items", "marian_mensaje.png")
                cellphone_img = pygame.image.load(cellphone_path).convert_alpha()
            ...
            if cellphone_img:
                ...
                img_y = (self.screen.get_height() - cellphone_img.get_height()) // 2 - 50
                self.screen.blit(cellphone_img, (img_x, img_y))
            # Mostrar el diálogo debajo de la imagen
        ...
        for sprite in self.player_group:
            world_surface.blit(sprite.image, (sprite.rect.x - camera_offset[0],
