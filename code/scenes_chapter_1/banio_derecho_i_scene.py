...
class BanioDerechoIScene(Scene):
    def __init__(self, screen, player):
        super().__init__(screen)
        self.map = Map("bathroom_IR.png")
        self.obstacles = get_collisions("banio_derecho_i")
        self.player = player
        self.player_group = pygame.sprite.Group(self.player)
        self.DEFAULT_ZOOM = 2
        self.current_map = "banio_derecho_i"
    ...
    def update(self, dt):
        ...
        door_interior_to_edificio_i = pygame.Rect(0, 210, 5, 126)
        if self.player.rect.colliderect(door_interior_to_edificio_i):
            self.player.rect.center = (421, 138)
