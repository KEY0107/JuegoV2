...
class BanioIzquierdoI(Scene):
    def __init__(self, screen, player):
        super().__init__(screen)
        # Cargar los assets para el interior del edificio H
        self.map = Map("bathroom_IL.png")
        # Se asume que las colisiones están definidas con la clave "edificio_h"
        self.obstacles = get_collisions("banio_izquierdo_i")
        self.player = player
        self.player_group = pygame.sprite.Group(self.player)
        self.DEFAULT_ZOOM = 2
        self.current_map = "banio_izquierdo_i"
    ...
    def update(self, dt):
        ...
        door_interior_to_edificio_i = pygame.Rect(547, 232, 5, 93)
        if self.player.rect.colliderect(door_interior_to_edificio_i):
            self.player.rect.center = (231, 138)
