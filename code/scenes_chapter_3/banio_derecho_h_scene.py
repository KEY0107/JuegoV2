...
class BanioDerechoHScene(Scene):
    def __init__(self, screen, player):
        super().__init__(screen)
        self.map = Map("bathroom_HR.png")

        self.obstacles = get_collisions("banio_derecho_h")
        self.player = player
        self.player_group = pygame.sprite.Group(self.player)
        self.DEFAULT_ZOOM = 2
        self.current_map = "banio_derecho_h"
    ...
    def update(self, dt):
        ...
        # transicion para regresar al pasillo
        door_interior_to_edificio_h = pygame.Rect(0, 210, 5, 126)
        if self.player.rect.colliderect(door_interior_to_edificio_h):
            self.player.rect.center = (415, 460)
